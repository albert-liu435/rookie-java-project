package com.rookie.bigdata.algorithm.structures.LinkedList.sqlist;
...
public class SeqList<T> implements ISeqList<T> {
    ...
    public boolean add(int index, T data) {
        ...
        if (this.length == table.length) {
            ...
            //对原来的数组进行成倍扩容，并把原来的数组赋值到新的数组下面
            this.table = new Object[this.length * 2];

            //把原来的数组进行成倍扩容，并把原数组的元素赋值到新数组里面
    ...
    public boolean add(T data) {
        return this.add(this.length, data);
    }
